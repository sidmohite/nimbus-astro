...
parser.add_argument('--survey_file', required = True, metavar = 'SURVEY_FILE',
                   help = 'File containing field, pixel and extinction\
                   specific information for the survey.')
parser.add_argument('--skymap_file', required = True, metavar = 'SKYMAP_FILE',
                   help = 'Skymap file for the event.')
parser.add_argument('--infield_likelihoods_path', required = True,
                   metavar = 'INFIELD_LIKELIHOODS_PATH',
                   help = 'Path to files containing sample likelihood values\
                   for each field. The code expects files to be named using a\
                   common string pattern (see below) with the field number\
                   appended at the end.')
