...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description = 'Combine the in-field\
                                    likelihoods to construct the final\
                                    posterior',
                                    formatter_class =\
                                    argparse.ArgumentDefaultsHelpFormatter,
                                    fromfile_prefix_chars='@')
    parser.add_argument('--sample_file', required = True,
                       metavar = 'SAMPLE_FILE', help = 'File containing sample\
                       points')
    parser.add_argument('--field_probs_file', required = True,
                       metavar = 'FIELD_PROB_FILE',
                       help = 'File containing the total sky probability for\
                       each field.')
    parser.add_argument('--infield_likelihoods_str', required = True,
                       ...
                       likelihood values for each field. The code expects\
                       files to be named using a common string pattern\
                       (see below) with the field number appended at the end.')
    parser.add_argument('--coverage_fraction', type = float, default=0,
                       metavar = 'COV_FRAC', help = 'Assumed pseudo fraction\
                       of the event skymap that is surveyed by the telescope.\
                       Range(0-1)')
    parser.add_argument('--P_A', required = True, type = float,
                       metavar = 'P_ASTRO', help = 'Probability of the event\
                       being astrophysical.Range(0-1)')
